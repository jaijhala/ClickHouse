...
// FIXME: include jemalloc.h instead
    ...
    const size_t /*num_streams*/)
{
    ...
    for (UInt64 bin = 0; bin < small_bins_count; ++bin, ++bin_index)
    {
        ...
        size_t col_num = 0;
        res_columns[col_num++]->insert(bin_index);
        res_columns[col_num++]->insert(0);
        res_columns[col_num++]->insert(size);
        res_columns[col_num++]->insert(nmalloc);
        res_columns[col_num++]->insert(ndalloc);
    }
